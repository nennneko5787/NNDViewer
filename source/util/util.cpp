...
std::string Util_convert_seconds_to_time(double input_seconds) {
	if (input_seconds == INFINITY) {
		return "INF"; // for debug purpose
	}
	int hours = 0;
	int minutes = 0;
	int seconds = 0;
...
// truncate and wrap into at most `max_lines` lines so that each line fit in `max_width` if drawn with the size of
// `x_size` x `y_size` assumes input_str doesn't contain any linebreaks
std::vector<std::string> truncate_str(std::string input_str, int max_width, int max_lines, double x_size,
                                      double y_size) {
	u32 input[1024];
	...
	for (int line = 0; line < max_lines; line++) {
		...
		{ // get the number of words that fit in the line
			for (int i = head; i < m; i++) {
				...
				for (u64 c : words[i]) {
					cur_word_width += Draw_get_width_one(c, x_size);
				}
		...
		for (size_t i = word_start[head]; i < word_start[head + fit_word_num]; i++) {
			...
			if (cur_char >> 24) {
				cur_line.push_back(cur_char >> 24);
			}
			if (cur_char >> 16) {
				cur_line.push_back(cur_char >> 16 & 0xFF);
			}
			if (cur_char >> 8) {
				cur_line.push_back(cur_char >> 8 & 0xFF);
			}
