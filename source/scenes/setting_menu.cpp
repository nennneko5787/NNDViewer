...
, (std::function<std::string()>)[]() { return LOCALIZED(LANG_IT); }
	...
	return LOCALIZED(LCD_BRIGHTNESS); })
	...
	misc_tasks_request(TASK_SAVE_SETTINGS); }),
					...
					(new BarView(0, 0, 320, 40))
						...
						->set_title([] (const BarView &view) {
	return LOCALIZED(TIME_TO_TURN_OFF_LCD) + " : " +
	       (view.get_value() <= 309 ? std::to_string((int)view.get_value()) + " " + LOCALIZED(SECONDS)
	                                : LOCALIZED(NEVER_TURN_OFF)); })
						->set_on_release([] (const BarView &view) {
...
, (std::function<std::string()>)[]() { return LOCALIZED(ON); }
	...
	if (var_night_mode != view.selected_button) {
		...
		misc_tasks_request(TASK_SAVE_SETTINGS);
					...
					(new BarView(0, 0, 320, 35))
						...
						->set_title([] (const BarView &view) {
	return LOCALIZED(SCROLL_SPEED0) + " : " + double2str(var_dpad_scroll_speed0, 1); })
						->set_while_holding([] (const BarView &view) {
	...
	misc_tasks_request(TASK_SAVE_SETTINGS); }),
					...
					(new BarView(0, 0, 320, 35))
						...
						->set_title([] (const BarView &view) {
	return LOCALIZED(SCROLL_SPEED1) + " : " + double2str(var_dpad_scroll_speed1, 1); })
						->set_while_holding([] (const BarView &view) {
	...
	misc_tasks_request(TASK_SAVE_SETTINGS); }),
					...
					(new BarView(0, 0, 320, 40))
						...
						->set_title([] (const BarView &view) {
	return LOCALIZED(SCROLL_SPEED_THRESHOLD) + " : " + double2str(var_dpad_scroll_speed1_threshold, 1); })
						->set_on_release([] (const BarView &view) {
	misc_tasks_request(TASK_SAVE_SETTINGS); }),
					...
					(new BarView(0, 0, 320, 40))
						...
						->set_title([] (const BarView &view) {
	return LOCALIZED(COMMUNITY_POST_IMAGE_SIZE) + " : " + std::to_string(var_community_image_size) + " px"; })
						->set_on_release([] (const BarView &view) {
	misc_tasks_request(TASK_SAVE_SETTINGS); }),
				...
				->set_views({
					...
					(new SelectorView(0, 0, 320, 35))
						->set_texts({
							(std::function<std::string ()>) []() { return LOCALIZED(OFF); },
							(std::function<std::string ()>) []() {
	return LOCALIZED(ONLY_IN_PLAYLIST); },
							(std::function<std::string ()>) []() {
	return LOCALIZED(ON); }
}, var_autoplay_level)
						->set_title([](const SelectorView &) {
	...
	return LOCALIZED(REMOVE_ALL_HISTORY); })
	...
	popup_view->set_buttons<std::function<std::string()>>(
	    ...
	    [](OverlayDialogView &, int button_pressed) {
		    if (button_pressed == 1) {
			    ...
			    toast_view->set_text((std::function<std::string()>)[]() { return LOCALIZED(ALL_HISTORY_REMOVED); })
			        ->set_x_alignment(TextView::XAlign::CENTER)
			        ->set_text_offset(0, -1)
...
main_view = (new VerticalListView(0, 0, 320))
                ->set_views({// 'Settings'
                             ...
                             // ---------------------
                             (new RuleView(0, 0, 320, SMALL_MARGIN * 2))->set_get_background_color([](const View &) {
	                             return DEFAULT_BACK_COLOR;
                             }),
                             main_tab_view})
...
update_worker_thread =
    threadCreate(update_worker_thread_func, (void *)(""), DEF_STACKSIZE, DEF_THREAD_PRIORITY_LOW, 1, false);
