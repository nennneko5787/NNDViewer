...
class RJson
{
  ...
  public:
	...
	static RJson parse(rapidjson::Document &data, const char *s, std::string &error)
	{
		...
		{
			error = "Parsing error " + std::to_string(data.GetParseError()) + " at " +
			        std::to_string(data.GetErrorOffset());
			return RJson();
		}
		else
		{
	...
	{
		return (*this)[str].is_valid();
	}

	const char *cstring_value() const
	{
		return json && json->IsString() ? json->GetString() : "";
	}
	std::string string_value() const
	{
		return cstring_value();
	}
	int int_value() const
	{
		return json && json->IsInt() ? json->GetInt() : 0;
	}
	bool bool_value() const
	{
		return json && json->IsBool() ? json->GetBool() : false;
	}
	...
	RJson operator[](const char *key) const
	{
		return json && json->IsObject() && json->HasMember(key) ? (*json)[key] : RJson();
	}
	RJson operator[](const std::string &str) const
	{
		return (*this)[str.c_str()];
	}
	RJson operator[](size_t index) const
	{
		return json && json->IsArray() ? (*json)[index] : RJson();
	}
