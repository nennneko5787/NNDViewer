	...
	if (do_thumbnail_update) {
		...
		int displayed_r = -1;
		{
			...
			for (int i = 0; i < (int)views.size(); i++) {
				...
				if (cur_y >= 0)
					displayed_l = std::min(displayed_l, i);
				cur_y += margin;
			}
			if (displayed_l > displayed_r)
				displayed_l = displayed_r = 0;
			else
				displayed_r++;
		}
		...
		for (int i = request_target_l; i < request_target_r; i++) {
			...
			if (cur_view)
				...
				if (cur_view)
					...
					priority_list[i - request_target_l] = {-1, -1};
			}
		}
