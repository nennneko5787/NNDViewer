	...
	void draw_() const override
	{
		if (!draw_order.size())
		{
			...
			for (auto view : views)
			{
				...
				if (x_right >= 0 && x_offset < 320)
					view->draw(x_offset, y0);
		...
		else
		{
			...
			for (size_t i = 0; i < views.size(); i++)
				x_pos[i + 1] = x_pos[i] + views[i]->get_width() + margin;
			for (auto i : draw_order)
				if (x_pos[i + 1] >= 0 && x_pos[i] < 320)
					views[i]->draw(x_pos[i], y0);
		}
	}
