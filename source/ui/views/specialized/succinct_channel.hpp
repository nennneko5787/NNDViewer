...
struct SuccinctChannelView : public FixedSizeView
{
  ...
  public:
	...
	SuccinctChannelView *set_name(const std::string &name)
	{ // mandatory
		this->name = name;
		return this;
	}
	SuccinctChannelView *set_auxiliary_lines(const std::vector<std::string> &auxiliary_lines)
	{ // mandatory
	...
	void draw_() const override
	{
		...
		thumbnail_draw(thumbnail_handle, x0 + (CHANNEL_ICON_WIDTH - CHANNEL_ICON_HEIGHT) / 2, y0, CHANNEL_ICON_HEIGHT,
		               CHANNEL_ICON_HEIGHT);
		float y = y0;
