...
struct View
{
  ...
  public:
	...
	void update(Hid_info key)
	{
		if (is_touchable)
		{
			bool inside_view = key.touch_x >= x0 && key.touch_x < x0 + get_width() && key.touch_y >= y0 &&
...
{
  public:
	FixedSizeView(double x0, double y0, double width, double height)
	    : View(x0, y0), FixedWidthView(x0, y0, width), FixedHeightView(x0, y0, height)
	{
	}
